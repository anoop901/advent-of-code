...
def terminal_output_to_unparsed_commands(terminal_output: str) -> list[UnparsedCommand]:
    ...
    for line in lines:
        ...
        if m:
            current_command = UnparsedCommand(command_line=m.group(1), output=[])
...
def directory_total_size(directory: Directory) -> int:
    return sum(
        directory_content_total_size(directory_content)
...
class Solution:
    ...
    def part1(self) -> int:
        return sum(
            size
            for size in (
                directory_total_size(directory)
    ...
    def part2(self) -> int:
        ...
        return min(
            (
                ...
                if total_used_space - size + UNUSED_SPACE_NEEDED
